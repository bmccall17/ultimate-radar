...
The rule is about the direction, not about the kind of data. Positions are the
...
and cannot lower a span's emission cost. It is not scored zero. It is not in the
sample.
...
anchored frame would leave the ramp's output looking like the tracker's own, which
is the same lie one frame over.
...
to make the artefact better, which is the point of AD-6, and the reader should see
...
12 yd on a stride of 8 keyframes, laid in through `ur.resolve` by exactly the route
a person's save takes. It runs each grader three times: with its exclusion switched
off, as it normally runs, and over `blind()` of the document. The last two must be
identical and the first must differ from them, or the probe never reached that
grader and the agreement says nothing.
...
evidence, and it is worth more, but only while the structure holds.
...
and removing exactly those leaves a sample of the frames it already handled.
Recall over a repaired possession goes up, honestly computed, and means less.

A number measured after any hand placement is **bounded**: an upper bound, not a
result. It joins declared, measured and observed in `CONTEXT.md` as a fourth
answer to what kind of number this is, and it carries one obligation. **Print the
...
`identity_switches_caught: "2 of 2"` which was the swap detector agreeing with the
analysis that produced it, and a derived attacking direction written back as an
observation (AD-10, #5). Repair mode is the fourth and the most dangerous, because
it is the feature that makes a person want to correct a lot of frames. Every
previous instance was caught by somebody noticing. This one is caught by
construction.
